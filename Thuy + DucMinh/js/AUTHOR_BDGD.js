...
async function fetchViewedArticles(userId, token, page = 1, limit = 6, category = 'Tất cả', keyword = '') {
    try {
        const queryParams = new URLSearchParams({ page, limit });
        if (category !== 'Tất cả') queryParams.append('category', category);
        if (keyword) queryParams.append('keyword', keyword);

        const res = await fetch(`${window.API_BASE_URL}/api/articles/viewed/${userId}/?${queryParams.toString()}`, {
            headers: { 'Accept': 'application/json', 'Authorization': `Bearer ${token.trim()}` }
        });
        if (!res.ok) {
            const errorText = await res.text();
        ...
        return { viewedArticles: [], total: 0, page: 1, limit: 6 };
    }
}
...
function populateTable(viewedArticles, tableBodyId, currentPage, category, keyword) {
    ...
    if (!Array.isArray(viewedArticles)) {
        console.error('Invalid viewedArticles array:', viewedArticles);
    ...
    viewedArticles.forEach(history => {
        ...
        row.innerHTML = `
            ...
            <td>
                ...
                <i></i>
                <i class="fa-solid fa-trash-can delete-btn" data-history-id="${history._id || ''}"></i>
            </td>
        `;
...
let viewedPagination;

function resetPagination(total, page) {
    viewedPagination = new Pagination('.pagination', total, 6, (newPage) => {
        const params = new URLSearchParams(window.location.search);
...
async function initializeTables(page = 1, category = 'Tất cả', keyword = '') {
    ...
    const viewedData = await fetchViewedArticles(window.currentUser.id, token, page, 6, urlCategory, urlKeyword);
    console.log('Viewed data:', viewedData);
    console.log('Current filters - category:', urlCategory, 'keyword:', urlKeyword);

    populateTable(viewedData.viewedArticles, 'table-body', page, urlCategory, urlKeyword);

    resetPagination(viewedData.total, page);
}
